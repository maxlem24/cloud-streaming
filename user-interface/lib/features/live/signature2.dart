...
class Signature {
  ...
  static Future<ProcessResult> runFile(List<String> args) async {
    ...
    print('🔧 Running JAR: $jarPath with args ${args.join(",")}');
  ...
  static Future<void> owner_init(String id ,String base64) async {
    ...
    print("OwnerInit:" + result.stderr);
  }

  static Future<List<String>> owner_sign(String path,String frameId) async {
    var result = await runFile(['owner', 'sign', path, frameId]);
    if (result.stderr.toString().isNotEmpty) {
      print(result.stderr);
      return [result.stderr];
    }
